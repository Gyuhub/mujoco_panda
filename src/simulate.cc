...
// prepare to render
void prepare(std::shared_ptr<Controller> &node) {
  ...
  mjvGeom G_cur, G_tar;
  G_cur.type=mjGEOM_SPHERE;
  G_cur.dataid = -1;
  G_cur.objtype = mjOBJ_UNKNOWN;
  G_cur.objid = -1;
  G_cur.category = mjCAT_DECOR;
  G_cur.texid = -1;
  G_cur.texuniform = 0;
  G_cur.texrepeat[0] = 1;
  G_cur.texrepeat[1] = 1;
  ...
  G_tar.type=mjGEOM_SPHERE;
  G_tar.dataid = -1;
  G_tar.objtype = mjOBJ_UNKNOWN;
  G_tar.objid = -1;
  G_tar.category = mjCAT_DECOR;
  G_tar.texid = -1;
  G_tar.texuniform = 0;
  G_tar.texrepeat[0] = 1;
  G_tar.texrepeat[1] = 1;
  ...
  Eigen::Vector3d cur, tar;
  cur = node->get_current();
  tar = node->get_target();
  G_cur.size[0] = 0;
  G_cur.size[1] = 0;
  G_cur.size[2] = 0;
  G_cur.rgba[0] = 1.0;
  G_cur.rgba[1] = G_cur.rgba[2] = 0;
  G_cur.rgba[3] = 1;
  G_cur.pos[0] = cur(0);
  G_cur.pos[1] = cur(1);
  G_cur.pos[2] = cur(2);
  ...
  G_tar.mat[1] = 1;
  G_tar.mat[2] = 1;
  memset(G_tar.label, 0, 100);
  G_tar.label[0] = '.';

  mjvGeom *G_cur_ptr = scn.geoms + scn.ngeom;
  *G_cur_ptr = G_cur;
  scn.ngeom++;
  mjvGeom *G_tar_ptr = scn.geoms + scn.ngeom;
  *G_tar_ptr = G_tar;
  scn.ngeom++;
...
void simulate(std::shared_ptr<Controller> &node) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        node->get_state(d->time, d->qpos, d->qvel);
        node->update();
        node->set_command(d->ctrl, d->qpos);
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // inject noise
...
void spin_ros(std::shared_ptr<Controller> &node)
{
  rclcpp::spin(node);
  return;
}
...
{
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Caught Signal[%d]!", signo);
  rclcpp::shutdown();
...
int main(int argc, const char** argv) {
  ...
  std::string model_file_path = home_path + "/rp_ws/src/mujoco_panda/models/franka_panda.xml";

  strcpy(filename, model_file_path.c_str());
  settings.loadrequest = 2;
  // ROS2 class
  auto node = std::make_shared<Controller>("panda");

  // start simulation thread
  std::thread simthread(simulate, std::ref(node));
  std::thread rosthread(spin_ros, std::ref(node));
